...
- **Distribution posture:** Copyrighted liturgical text in `data/` and `sources/` is gitignored. Only public-domain KJV, `data/corrections.json`, `data/paragraphs.json`, and `data/monarch.json` are committed. `web/data` is a symlink to `../data` dereferenced into `dist/` on build. Per ADR 0025, clients use the standalone Backend-For-Frontend (BFF) API for daily calendar and scripture payloads rather than bundling the full year's lectionary or scripture books.
