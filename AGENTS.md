...
node tools/audit_rubric_repeats.cjs # rubric printed twice in one form, or a doxology cue no other form shares
...
| `audit_rubric_repeats.cjs` | Compares the corpus against itself, the way `check_conservation.py` does for extraction: does any rubric print twice within one form, and does any form's doxology cue disagree with every other form's (legitimate seasonal/ordinary phrasing variants always span at least an MP/EP pair — a phrasing no other form shares is a typo, not a variant). Found #97's two book typos with zero false positives. Advisory. |
