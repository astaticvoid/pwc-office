...
| Reading introduction | "A Reading from the Daily Office Lectionary, the Weekday Eucharistic Lectionary, or the Revised Common Lectionary Daily Readings is read." (as printed) | "One or two readings are read." | Review: drop the named lectionaries and align with Synod/ADR 0019 item 7. Applied as `adr0019-item3-reading-introduction` in `data/corrections.json`, `source: upstream-review`. A substring correction, so the reflection prompt sharing the printed paragraph stays attached. ADR 0019 item 3 & 7. |
