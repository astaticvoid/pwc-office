...
Run from repo root:
  ...
  --skip-existing       Skip dates that already have entries in data/lectionary/
"""
...
def _request_html(d: date) -> str | None:
    """HTTP GET for one day. Returns raw HTML or None if not available."""
...
def fetch_day_html(d: date, *, delay: float = 1.0, no_cache: bool = False) -> str | None:
    """
    Return the raw HTML for a day, using cache when available.
    Returns None if the date has no data on the server.
    """
...
# Matches " - HD", " - Mem", " - Com", " - PF" (abbreviated CSV style) and
# " - Holy Day", " - Memorial", " - Commemoration", " - Principal Feast" (full).
